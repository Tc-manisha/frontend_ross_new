...
export default function Documents(props) {
  ...
  const renderLink = (e) => {
    ...
    return(
       <span onClick={()=> (user?.user_type == 0 || (user?.user_type == 2 && user?.sub_admin != "" && privilege?.includes("document-details")))
        && navigate(`/account-document-details/${id}`)}
       className={(user?.user_type == 0 || (user?.user_type == 2 && user?.sub_admin != "" && privilege?.includes("document-details")))
       ? "link" : ""}
       >
      {e?.value}
        </span>
          ...
          {(props?.documentData && props?.documentData?.allAccountDocuments && props?.type == "Account") && (
            <>
              ...
              <Box className="text-left pt-3 pb-1 d-flex">
                ...
                {(user?.user_type == 0 ||
               (user?.user_type == 2 && user?.sub_admin != "" && privilege?.includes("new-document"))) && ( 
                <span className='' style={{ marginLeft: 'auto' }}>
                  <button type="button"
                    onClick={() => {
                      navigate("/account-document-upload", {
