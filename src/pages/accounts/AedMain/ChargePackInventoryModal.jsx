...
const ChargePackInventoryModal = ({
  inventoryModal,
  setInventoryModal,
  secName,
  api,
      ...
      if (response.status === true) {
        ...
        if (data?.batteryInfo) {
          for (let invent = data.batteryInfo.length - 1; invent >= 0; invent--) {
            ...
            for (let index2 = 0; index2 < AedFormData[index].charge_pack_list.length; index2++) {
              const element2 = AedFormData[index].charge_pack_list[index2];
  ...
  const handleSubmit = async (e) => {
    ...
    if (secName) {
      ...
      if (fd[index]?.charge_pack_list && fd[index].charge_pack_list[Bindex]) {

        fd[index].charge_pack_list[Bindex].battery_info.battery_udi = selectedBatteryRows[0]?.battery_udi;
        fd[index].charge_pack_list[Bindex].battery_info.charge_pak_uid = selectedBatteryRows[0]?.charge_pak_uid;
        fd[index].charge_pack_list[Bindex].battery_info.battery_lot = selectedBatteryRows[0]?.battery_lot;
        fd[index].charge_pack_list[Bindex].battery_info.battery_expiration = selectedBatteryRows[0]?.battery_expiration;
        fd[index].charge_pack_list[Bindex].battery_info.manufactured_date = selectedBatteryRows[0]?.manufactured_date;
        fd[index].charge_pack_list[Bindex].battery_info.install_date = selectedBatteryRows[0]?.install_date;
        fd[index].charge_pack_list[Bindex].battery_info.battery_type_id = selectedBatteryRows[0]?.battery_type_id;

        setSelectedBatteryTypeId(selectedBatteryRows[0]?.battery_type_id)

        fd[index].charge_pack_list[Bindex].inventory_id = selectedBatteryRows[0]?.sbid;

        fd[index].charge_pack_list[Bindex].pad_1_info.is_pediatric = selectedBatteryRows[0]?.pad1?.is_pediatric;
        fd[index].charge_pack_list[Bindex].pad_1_info.pad_type_id = selectedBatteryRows[0]?.pad1?.pad_type_id;
        fd[index].charge_pack_list[Bindex].pad_1_info.pad_expiration = selectedBatteryRows[0]?.pad1?.pad_expiration;
        fd[index].charge_pack_list[Bindex].pad_1_info.pad_lot = selectedBatteryRows[0]?.pad1?.pad_lot;
        fd[index].charge_pack_list[Bindex].pad_1_info.pad_udi = selectedBatteryRows[0]?.pad1?.pad_udi;

        fd[index].charge_pack_list[Bindex].pad_2_info.is_pediatric = selectedBatteryRows[0]?.pad2?.is_pediatric;
        fd[index].charge_pack_list[Bindex].pad_2_info.pad_type_id = selectedBatteryRows[0]?.pad2?.pad_type_id;
        fd[index].charge_pack_list[Bindex].pad_2_info.pad_expiration = selectedBatteryRows[0]?.pad2?.pad_expiration;
        fd[index].charge_pack_list[Bindex].pad_2_info.pad_lot = selectedBatteryRows[0]?.pad2?.pad_lot;
        fd[index].charge_pack_list[Bindex].pad_2_info.pad_udi = selectedBatteryRows[0]?.pad2?.pad_udi;
  ...
  const renderCheckBox = (e) => {
    ...
    // let chargePackRowIndex = AedFormData[index].charge_pack_list[Bindex]?.inventory_id
    // const is_default_checked = secName ? chargePackRowIndex === e.data.sbid : '';
  ...
  const renderDateBatteryCell = (battery) => {
    if (secName) {
      const dateField = battery?.battery_expiration
      ...
      return (
        <div>
          <img
            src={imageSourceMap?.battery_expiration}
            alt={dateField}
            style={{
...
export default ChargePackInventoryModal;