...
import EditAedSubForms from "./EditAedSubForms";
// GatewayInformation
const NewEditAedForm = () => {
  ...
  const [allAedData,setAllAedData] = useState({});
  ...
  const onLoad = async () => {
    ...
    const aedObj = {
        aed_data: res?.data?.data,
        all_pads: res?.data?.allPads,
        battery_info: res?.data?.batteryInfo
    }
    setAllAedData(aedObj)
    ...
    console.log({chck1: (AEDFromData)});
    AEDFromData.adult_pad_info = AEDFromData?.adult_pad_info!=undefined ? JSON.parse(AEDFromData?.adult_pad_info) : {};
        ...
        <>
          <div className="mt-4" style={{ paddingInline: "45px" }}>
            <form class="" onSubmit={handleSubmit}>
              ...
              <EditAedSubForms
                EditAedData={allAedData}
                aedFormData={aedFormData} 
                setAedFormData={setAedFormData}
              />
