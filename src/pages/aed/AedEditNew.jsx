...
import EditAedForm from "./AedEditComp/EditAedForm";
import EditParentBatteryInfo from "./AedEditComp/EditParentBatteryInfo";
import EditChargePakInfoSections from "./AedEditComp/EditChargePakInfoSections";
import EditPadInfoSection from "./AedEditComp/EditPadInfoSections";
...
const AedEditNew = () => {
  ...
  const [oldFormdData, setOldFormdData] = useState({ ...formData });
  const [removedData, setRemovedData] = useState({});
  const [deletedDniData, setDeletedDniData] = useState({});
  

  // const Removespare_battery_info = useCallback((index, section) => {
  //   const arr = {...formData};

  //   // setFormData(prevFormData => {
  //   //   const updatedFormData = { ...prevFormData };

  //   //   if (Array.isArray(updatedFormData[section])) {
  //   //     const [removedItem] = updatedFormData[section].splice(index, 1);

  //   //     if (removedItem && !removedItem.hasOwnProperty('is_new')) {
  //   //       setRemovedData(prevRemovedData => {
  //   //         const updatedRemovedData = { ...prevRemovedData };
  //   //         const currentSectionData = updatedRemovedData[section] || [];

  //   //         // Only update state if there is a change
  //   //         if (!currentSectionData.includes(removedItem)) {
  //   //           updatedRemovedData[section] = [...currentSectionData, removedItem];
  //   //         }

  //   //         return updatedRemovedData;
  //   //       });
  //   //     }
  //   //   }
  ...
  //   // });
  // }, []);
  const [removewDatalist,setRemovedDataList] = useState([]);

  const Removespare_battery_info = (index, section) => {
    let arr = { ...formData };
    let arr2 = [...removewDatalist];
    if (Array.isArray(arr[section])) {
      const removedData = arr[section].splice(index, 1)[0];
        if (!removedData.hasOwnProperty('is_new')) {
          arr2.push({section:section,data:removedData});
          setRemovedDataList(arr2); 
          // setRemovedData(prev => ({
          //     ...prev,
          //     [section]: prev[section] ? [...prev[section], removedData] : [removedData]
          // }));
      }
    }
  ...
  const resetRemovedData = (section) => {
    setFormData(prev => ({
      ...prev,
      [section]: [...oldFormdData[section]]
    }));
  };
 

const handleDNI = (item, section) => {
  let arr = { ...formData };
  let dniData = {...deletedDniData.current}; //deletedDniData ? { ...deletedDniData } : deletedDniData;
  if (Array.isArray(arr[section])) {
    const itemIndex = arr[section].findIndex((i) => i.bid === item.bid);
    if (itemIndex !== -1) {
      const updatedSection = arr[section].filter((_, index) => index !== itemIndex);
      const removedData = arr[section][itemIndex];
      arr[section] = updatedSection;
      if (!Array.isArray(dniData[section])) {
        dniData[section] = [];
      }

      dniData[section] = [...dniData[section], removedData];
      dniData[section][dniData[section].length - 1].created_date = "";
    }
  }
  deletedDniData.current = dniData;
  setDeletedDniData(dniData);
  setFormData(arr);
}; // Removed deletedDniData from dependencies
  ...
  const addRemoveChargePakInfo = (type, keyName) => {
    if (type === "add") {
      ...
      if (keyName && arr[keyName] && crr) {
        ...
        console.log('>>>>>>>> add keyName : ', crr, keyName);
        setFormData(arr);
      }
    } else {
      let arr = { ...formData };
  ...
  const handleSubmit = async (e) => {
    ...
    let sendFormData = {
      ...
      useSbidInventory:[],
      useSpidInventory:[],
      dni_array_list: deletedDniData
    };
    console.log({removewDatalist,deletedDniData});
    // deletedDniData // removewDatalist // charge_pak_arr
    // sendFormData.useSbidInventory  = formData.useSbidInventory;
    console.log({CHeck1:formData.useSbidInventory})
    console.log({CHeck2:formData.useSpidInventory})
    // console.log({removewDatalist})
    // useSpidInventory
    // useSpidInventory
    // console.log()
          ...
          <div className="mt-4" style={{ paddingInline: "45px" }}>
            <form class="" onSubmit={handleSubmit}>
              ...
              <EditParentBatteryInfo
                ...
                is_edit={true}
                Removespare_battery_info={Removespare_battery_info}
                resetRemovedData={resetRemovedData}
                handleDNI={handleDNI}
                removedData={removedData}
                deletedDniData={deletedDniData.current}
              />

              <EditChargePakInfoSections 
                addRemoveChargePakInfo={addRemoveChargePakInfo}
                formData={formData}
                setFormData={setFormData}
                handleCheckBox={handleCheckBox}
                handleInput={handleInput}
                addMorebattery_info={addMorebattery_info}
                padList={padList}
                all_condition_true={all_condition_true}
                BatteryList={BatteryList}
              />
