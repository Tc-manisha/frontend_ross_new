...
import AedSupport from "./Support";
import ServiceCheck from "../../../img/ServiceCheck.svg";
import AedDocuments from "./Documents";
import AEDChecksTbl from "./AedChecksTbl";
...
let privileges = getPermission();
  ...
  const filterTabsArray = () => {
    ...
    filteredContactsTabs.push({ name: 'Support', id: 'support-tab', 'order': 3 })
    filteredContactsTabs.push({ name: 'Documents', id: 'documents-tab', 'order': 4 })
      ...
      {showLoading ? (
        ...
        <>
          ...
          >
                ...
                {privileges?.includes('aed-service-check') && (
                  <button className="btn text-primary" type="button" onClick={handleServiceCheck}>
                    <img
                      ...
                      height={20}
                    />
                    <span className="ms-1" style={{marginLeft:"10px"}}>Service Check</span>
                  </button>
                )}
            ...
            {/* Notes */}
            {currentTab === filteredTabs.Notes &&
                  <>
                    <Notes aedId={aedId} account_id={userAccountId} privileges={privileges} />
                  </>
             }

            {currentTab === filteredTabs.AEDChecks && <AEDChecksTbl />}
            ...
            {filteredTabs.hasOwnProperty('Documents') && currentTab === filteredTabs.Documents && (
              <AedDocuments aedId={aedId} accountID={is_user === true ? userAccountId : aedData?.account_id} />
            )}
