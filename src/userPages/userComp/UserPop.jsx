...
import { DataSymbol, DollarsIcon, RoundCheck, TruckSymbol } from "../../helper/Icons";
...
export default function UserPops({
    // accountId,
    site_id = 0,
    contact_id = 0,
    type = "ACCOUNT",
    ...
    const {pop_id, userAccountId} = useParams();
    ...
    const fetchData = async () => {
        ...
        const res = await CallGETAPI('user/pop-list')
        if (res.data.status) {
    ...
    const renderCoverage = (e) => {
        ...
        const check = moment(rowData?.contract_start).add(1, 'year').format(DateFormate);
        const check2 = moment(rowData?.contract_start).format(DateFormate);
        // if(check==='Invalid date'){
        //   return '';
        // }

        return <div>{check2} {check ? " - " + check : ""}</div>
    }
    ...
    const renderContact = (e) => {
        ...
        console.log({ chec: typeof valData?.contract_officer, check: Array.isArray(valData?.contract_officer) });

        return <>
            <ul className={isUserPermission("pop-details") ? "link" : ""}
             style={{textDecoration:"none", listStyleType: "none", paddingLeft:"0px" }}>
                {valData?.contract_officer?.map((it, i) => (
                    <li key={i} style={{ textAlign: "left", paddingLeft:"0px" }}>
                        <span 
                         onClick={() => isUserPermission("pop-details") && navigate(`/user/${accountId}/contact-details/${it?.contact_id}`)} >
                            {it?.contact_name}
                            </span>
                            </li>
                ))}
    ...
    const renderContract = (e) => {
        ...
        return <>
            ...
            {data?.modification && <p className="m-0 ">Mod: {data?.modification}</p>}
        </>
    }

    let is_pop_details = false

    let permissions = getPermission(); localStorage.getItem('permissions')
    let permissionsArr = permissions.split(',')
    if (permissionsArr.includes('pop-details')) {
        is_pop_details = true
    }
                    ...
                    <button onClick={() => {
                        ...
                        <div className="d-flex">
                            {privileges.includes('new-pop') && (
                                <Link style={{ textDecoration: 'none', color:"#0C71C3" }} to={'/account/new-pop/' + accountId}>
                                    <img src={New} />
                                    New
                                </Link>
                            )}
                    ...
                    >
                        <Column
                            ...
                            cellRender={(e) => {
                                const rowData = e.data;
                                return (
                                    <span  className={isUserPermission("pop-details") ? "link" : ""}
                                        style={{ cursor: 'pointer', textDecoration:"none"}}
                                        onClick={() => { isUserPermission("pop-details") &&
                                            navigate(is_pop_details ? `/user/pop-details/${e.data.pop_id}/${accountId}` : '');
                                        }}
                                    >{parseInt(rowData.active) === 1 ?
                                        <span style={{ width: '10px' }}><RoundCheck /></span>
                                        : ""}
                                        &nbsp; {e.value}
                                    </span>
                                )
